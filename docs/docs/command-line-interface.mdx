...
:::note
If you run into out of memory issues during training, consider using the argument `rasa train --number-of-chunks <n>`.
This will split your dataset into `<n>` chunks and will process them one after each other.
