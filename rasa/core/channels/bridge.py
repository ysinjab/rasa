...
async def retrieve_webhook_calls(
    ...
    @sio.on("connect")
    async def connect():
        await sio.emit("subscribe", {"token": webhook.token})
        logger.info("subscribed to webhook")

    await sio.connect(f"{base_url}/ws", socketio_path="/ws/socket.io", headers=headers)
